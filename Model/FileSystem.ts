...
export class FileSystem{
    ...
    private currentDir:Node|null
    constructor(){
        this.root = new Node("root", "", null);
        this.currentDir = this.root;
    }

    public get getRoot():Node|null{
        return this.root;
    }

    public set setRoot(root:Node|null){
        this.root = root;
    }

    public get getCurrentDir():Node|null{
        return this.currentDir;
    }

    public set setCurrentDir(currentDir:Node|null){
    ...
    public cp(object1:string, object2:string):string{
        let content:string;
        if (this.currentDir!.getList!.search(object1) === null) return "no such file or directory.";
        //ディレクトリ直下へのコピー
    ...
    public treeHelper(node:Node|null, indent:string):string{
        let iterator:Node|null = node!.getList!.getHead;
        let ans:string = ``
        while(iterator != null){
